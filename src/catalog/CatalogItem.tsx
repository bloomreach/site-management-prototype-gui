...
import {
    ...
    reorder,
    simpleStringParameterTemplate, slugify
} from "./catalog-utils";
...
class CatalogItem extends React.Component<CatalogItemProps, CatalogItemState> {
        ...
        return (<AccordionActions>
            <Grid item sm={4}>
                <IconButton
                    ...
                    aria-label="export"
                    onClick={() => {
                        const installationActions = this.exportComponent();
    ...
    getAddOrEditFieldGroupDialog() {
        ...
        return <Dialog open={this.state.dialogOpen}>
            ...
            <DialogContent>
                <Form onChange={({formData}) => nextFieldGroupName = formData} formData={currentFieldGroupName}
                      uiSchema={fieldGroupUiSchema} schema={fieldGroupSchema as JSONSchema7}>
                    <></>
                </Form>
            </DialogContent>
            <DialogActions>
                                ...
                                {(provided: DroppableProvided) => (
                                    <Container {...provided.droppableProps}
                                                        ...
                                                        <div  {...provided.droppableProps}
                                                            ...
                                                            {this.state.parameters.map((parameter: ParameterType, index: number) => {
                                                                ...
                                                                return !isInFieldGroup && (
                                                                    <Draggable key={parameter.name}
                                                                               draggableId={parameter.name}
                                                                               index={index}>
                                                                        {(provided) => (
                                        ...
                                        {this.state.fieldGroups && this.state.fieldGroups.map((fieldGroup: FieldGroup, index: number) => {
                                            return (
                                                <Draggable key={fieldGroup.name}
                                                           draggableId={fieldGroup.name ? fieldGroup.name : index.toString()}
                                                           index={index}>
                                                    {(provided) => (
                                                        <Box
                                                            // @ts-ignore
                                                            ...
                                                            <DragDropContext onDragEnd={this.onDragEnd}>
                                                                <Droppable
                                                                    droppableId={fieldGroup.name ? fieldGroup.name : index.toString()}
                                                                    type={'fieldGroup_parameter'}>
                                                                        ...
                                                                        <div  {...provided.droppableProps}
                                                                            ...
                                                                            {fieldGroup.parameters?.map((fieldGroupParameterName, index) => {
                                                                                ...
                                                                                return fieldGroupParameter && (
                                                                                    <Draggable
                                                                                        key={fieldGroupParameter.name}
                                                                                        draggableId={fieldGroupParameter.name}
                                                                                        index={index}>
                        ...
                        <Drawer style={{maxWidth: '300px'}} anchor={'left'} open={this.state.drawerOpen}
                            ...
                            {this.state.selectedParameter && <>
                                ...
                                <Container>
                                    ...
                                    }} schema={getSchemaFromParameter(this.state.selectedParameter)}
                                          formData={this.state.selectedParameter}>
                                        <></>
                                    </Form>}
                                </Container>
                            </>}
    ...
    private exportComponent() {
        const component = {...this.state.componentDefinition};
        return [{
            type: 'site',
            path: `/channels/{channel_id}/component_groups/${this.props.group}`,
            method: 'PUT',
            body: {
            ...
            responseCode: 201,
            description: `create group: ${this.props.group}`
        } as InstallationAction,
            {
                type: 'site',
                path: `/channels/{channel_id}/component_groups/${this.props.group}/components/${slugify(component.label)}`,
                method: 'PUT',
                body: component,
                responseCode: 201,
